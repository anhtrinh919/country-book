...
The spread is intentionally **data‑driven**: a single `Country` data object feeds the layout. Rolling out a new country = author one data record, supply its assets, pick a **layout** + **palette** + **motif**, verify it fits. No bespoke layout work per country.

There are now **five interchangeable layout templates** (Atlas · Hero · Modular · Postcard · Timeline) and a **per‑country palette/motif**, precisely so 200+ pages don't feel identical. **The rotation rules in "Layout × Palette variety" below are mandatory** — read them before generating records.
...
## ⚠️ Layout × Palette variety — DO NOT ship a uniform book

The single biggest failure mode for this rollout is **making every page look the same**. Two independent axes create variety, and **both must rotate** — do not let either one cluster by region:

### 1. Layout (structure) — choose one of three per country
| `layout` | Look | Use it for |
|---|---|---|
| `atlas` | Flowing 2‑column; roundel masthead; tall side **data panel**; taped 3‑photo strip at the foot | the default workhorse |
| `hero` | Full‑width **photo banner** + accent **title bar**; horizontal **facts ribbon**; 2 inset photos right | countries with one iconic landmark/photo |
| `modular` | Tidy **boxed cards** with accent header tabs; clean 3‑up photo gallery | fact‑dense or "encyclopedia" feel |
| `postcard` | Bold **poster title band** + horizontal facts ribbon; right page leads with **postage‑stamp photos** (AIR‑MAIL postmarks) | playful, photo‑led countries |
| `timeline` | Vertical **accent spine** with numbered section **nodes** (Geography→Animals→Culture→History→Wonders) | countries with a strong story/journey |

### 2. Palette + motif (skin) — derived from the flag, one per country
`theme.accent` / `theme.accent2` come from the flag; `theme.paper`/`dot`/`ink`/`faint`/`line` are a country‑specific warm tint; `motif` is a signature ribbon (`sun`, `chevron`, `tricolore`, `papel`, `diamond`, `dots`, …).

### THE RULE (enforce in the generator)
- **Never give a whole continent/region the same `layout`.** Rotate so consecutive countries — and neighbours within a region — differ. A simple, safe approach: assign layouts round‑robin in reading order, e.g. `atlas, hero, modular, atlas, hero, modular, …`, then nudge so no two adjacent spreads share a layout.
- **Every country gets its OWN palette** from its flag — do **not** reuse one continent‑wide colour. (Egypt sand‑gold, Italy green, Brazil emerald, Australia ochre‑red, etc.) Two greens are fine on non‑adjacent pages; avoid identical accents back‑to‑back.
- **Pick `motif` for cultural fit, independent of layout** (e.g. Italy→`tricolore`, Mexico→`papel`, Australia→`dots`). Motif ≠ layout ≠ palette — mixing all three is what makes the book feel rich.
- Net effect: flipping any two facing pages, the reader should see a **different structure AND a different colour world**.

**Worked example (this bundle's 6 continents):**

| Continent | Country | `layout` | `theme.accent` / `accent2` | `motif` |
|---|---|---|---|---|
| Asia | Japan | atlas | `#C0202E` / `#23406b` | sun |
| Africa | Egypt | hero | `#B5862A` / `#1C7F86` | chevron |
| Europe | Italy | modular | `#1E7A43` / `#B5202F` | tricolore |
| N. America | Mexico | postcard | `#D1402B` / `#1E8A7B` | papel |
| S. America | Brazil | timeline | `#159A4C` / `#E0A800` | diamond |
| Oceania | Australia | modular | `#C0532A` / `#2C6FB0` | dots |

Note the layout column never repeats twice in a row, and every palette is unique. Replicate this discipline across all 195.

Adding a 4th/5th layout later = one new `LayoutX` component + an entry in the `LAYOUTS` dispatcher in `country-spread.jsx`; no data changes needed.
...
| `langName` | string | 1 | ≤ 16 | language shown in the phrasebook tick (e.g. "Aussie English") |
| `stop` | int | 1 | 1–195 | position in the world series (Explorer's Log stamp) |
| `layout` | enum | 1 | `atlas` \| `hero` \| `modular` | **which template renders it — ROTATE, never uniform per region** (see Layout × Palette variety) |
| `motif` | enum | 1 | `sun`,`chevron`,`tricolore`,`papel`,`diamond`,`dots` | signature ribbon; chosen for cultural fit, independent of layout |
| `theme` | object | 1 | — | per‑country palette: `paper,dot,ink,faint,line,accent,accent2` (accents from the flag) |
| `titleSize` | number | 0–1 | — | optional masthead px override for long names (default 70; AUSTRALIA=50) |
...
**Layout-library system (the current template — build from these):**
- `Country Templates.html` — gallery host: mounts React + Babel, loads the three files below, renders all continents on a pan/zoom canvas (focus any spread with click; ←/→/Esc).
- `country-spread.jsx` — **the layout library**: shared themed components (`Tick`, `SecHead`, `Prose`, `PhotoSlot`, `PhotoBox`, `Stamp`, `DataPanel`, `Roundel`, `Motif`, `MythBox`, `WowList`, `Phrasebook`, `Card`, `EdgeSpine`) + the three layouts (`Atlas`, `Hero`, `Modular`) + the `LAYOUTS` dispatcher + `CountrySpread`. Palette/motif applied via CSS variables set on the spread root from `C.theme`.
- `country-data.jsx` — all six country records (Japan + 5) in the full schema incl. `theme`/`motif`/`layout`. The model for authoring the other ~189.
- `design-canvas.jsx` — the gallery's pan/zoom canvas (presentation only; not part of a printed page).

**Schema & single-spread reference:**
- `country.schema.json` — JSON Schema (counts, char limits, `layout`/`motif`/`theme` enums & fields).
- `data.japan.json` — Japan as a clean JSON record (example payload).
- `Japan Country Book.html` + `japan-shared.jsx` + `japan-atlas.jsx` — the original **single‑country** Atlas reference (real CC photos, print/A3 host). Superseded by the layout-library files above, kept as a focused reference.
- `japan-spread-reference.png` — rendered screenshot of the finished Japan spread.

### Files (original notes)
...
1. Port `country-spread.jsx` into the target stack as a reusable `CountrySpread({C})` with its `LAYOUTS` dispatcher (the three layouts share themed components; palette comes from `C.theme` via CSS variables).
2. Author one JSON record per country against `country.schema.json` (the schema enforces counts, char limits, and the `layout`/`motif`/`theme` shape).
3. **Assign variety deliberately:** pull `accent`/`accent2` from the flag, set a country-specific `paper` tint, choose a `motif` for cultural fit, and **rotate `layout`** so no region is uniform and no two adjacent spreads match (see *Layout × Palette variety*).
4. Pipeline: validate record → fetch/clip flag SVG → place 4 self‑hosted CC photos → render chosen layout → **assert overflow=0 on both pages** → export A3 PDF.
5. Editorial pass per country: facts checked against sources, 2 myths chosen, population year current.
