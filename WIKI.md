<!-- wiki-generated -->
# Project WIKI — country-book
*Seeded from global wiki on 2026-06-12. Relevant to: react, react-dom, react-router-dom, react-simple-maps, topojson-client, world-atlas, @types/node, @types/react, @types/react-dom, @types/react-simple-maps, @types/topojson-client, @vitejs/plugin-react, ajv, pdf-lib, playwright, tsx, typescript, vite. 30 entries.*

## Typescript

### Phase 1: hybrid runtime split (headless core + helm bridge + browser UI) is the correct dogfood architecture for Electron apps
Phase 1 replaced the pure Electron-renderer approach with a hybrid: a Node core runs headlessly (no Electron window), exposes HTTP/WS via a helm bridge, and the UI is a browser pointed at localhost. Electron just packages and ships the stack. The non-obvious gain: the localhost UI is drivable by Claude Code browser tools during development and QA without Electron being open — which was the actual blocker on the prior architecture. Any future phase touching the runtime must understand this: headless core and browser UI are separate processes talking over the local network, not Electron IPC. The production .dmg packages both and opens the browser automatically.
...
### pdf-lib blob iframe renders grey in headless screenshots — not a bug
When taking headless browser screenshots of a page with a blob: URL loaded in an iframe (PDF preview), the iframe renders as solid grey. This is expected headless browser behavior — Chrome headless cannot render PDF blobs in iframes in screenshots. Verify the blob URL is valid and the iframe src is set correctly instead of relying on the screenshot to confirm PDF rendering.

## Javascript

### pdf-lib in-browser: disabled HTML button blocks real clicks but not programmatic .click() or dispatchEvent()
When testing validation gates with browse/headless tools, dispatching a MouseEvent directly to a disabled button fires the handler anyway — the disabled state only blocks real user interaction. Test validation blocking by checking button.disabled programmatically rather than simulating user clicks. This tripped up a test that tried to click a disabled generate button and got to the preview screen unexpectedly.

## Phase-wrap

### Phase 1 wrap: zero-process price-tag app — Excel-baked fidelity without Excel at runtime
Phase 1 shipped a pure file:// HTML/JS app (9.6MB dist) rendering 56 price-tag variants pixel-faithful to MKT's Excel template. Key architecture: dev-side bake renders backgrounds with Excel itself (sentinel-capture for positions, device-space via tm·cm composition), runtime overlays text with pdf-lib+fontkit. All Excel renders cached in bake/work/ — --reuse-renders regenerates packed assets with zero Excel launches. Contract keys (slugs) are baked artifacts; type classification is one engine function. Remaining: Windows PC validation (Phase 2), pixel ground-truth beyond A4 KM ngang needs one batched Excel export with user permission.
